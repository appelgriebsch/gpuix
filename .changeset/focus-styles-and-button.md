...
- default keyboard focus look, with no ring: a focusable control (`Button`, a `tabIndex` div) dims to 50% opacity; `<input>` and `<textarea>` draw nothing, the caret already shows focus. Any `focusVisible` replaces it, `focusVisible: {}` turns it off
